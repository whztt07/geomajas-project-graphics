...
public class LabeledPathClosedTest extends GraphicsMockSetup {
	...
	public void testConstructorCreatesRoles() throws Exception {
		...
		// fillable
		Assert.assertTrue(labeledPath.hasRole(Fillable.TYPE));
		Fillable fillable = labeledPath.getRole(Fillable.TYPE);
	...
	public void testCloneObjectFillable() throws Exception {
		...
		String labelString = "labelString";
		labeledPath = createLabeledPath(coordinates, labelString);
		Fillable expected = labeledPath.getRole(Fillable.TYPE);
		...
		LabeledPath labeledPathClone = assertIsCorrectObject(clone);
		Assert.assertTrue(labeledPathClone.hasRole(Fillable.TYPE));
		assertRoleEqualityFillable(expected, labeledPathClone.getRole(Fillable.TYPE));
	}
...
//		expected.setUserBounds(new Bbox(15, 20, 5, 5));
	...
	private LabeledPath createLabeledPath(Coordinate[] coordinates, String label) {
		return new LabeledPath(coordinates, true, label);
	}
